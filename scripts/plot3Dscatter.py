...
data = np.loadtxt("./TrapRatio_mutBiasSep_mutEW_voltageRatio.analysis",delimiter=",")
fig = plt.figure()
...
half_sigma = np.amax(data[:,3]) - np.std(data[:,3])/2
dat_half_sigma = []
for i in data:
...
one_sigma = np.amax(data[:,3]) - np.std(data[:,3])
dat_one_sigma = []
for i in data:
